...
export default function MentorLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const { error } = await login(email, password, 'mentor')
      if (error) {
    ...
    <div className="min-h-screen bg-linear-to-br from-purple-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">Mentor Login</h1>
          <p className="text-gray-600 dark:text-gray-400">Access your mentor dashboard</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-100 dark:border-gray-700">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Link href="/auth/mentor/forgot-password" className="text-sm text-purple-600 dark:text-purple-400 hover:underline">
                  Forgot password?
                </Link>
              </div>
              <input
            ...
            >
              {submitting ? 'Signing in...' : 'Sign in as Mentor'}
            </button>
          </form>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              ...
              <Link href="/auth/mentor/signup" className="text-purple-600 dark:text-purple-400 font-semibold hover:underline">
                Sign up
              </Link>
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
              Are you a student?{' '}
              <Link href="/auth/student/login" className="text-indigo-600 dark:text-indigo-400 font-semibold hover:underline">
                Student Login
              </Link>
            </p>
          </div>
